...
describe('runner scores', () => {
  ...
  it('scores 5 km finishes against the own-gender winner and averages them into the rank', () => {
    const scored = scoredRuns(MALE_RUNS, { gender: Gender.male, winners });
    ...
    expect(scoredRuns(MALE_RUNS, { gender: Gender.female, winners }), 'a female run has no female winner at these events').toHaveLength(2);
  ...
  it('weights the top window scores by freshness and trims the list to the top count', () => {
    ...
    expect(formIndex(scoredRuns(TRIM_RUNS, { gender: Gender.male, winners: winnerTimesBySlug(TRIM_EVENTS) }), SCORES_TODAY_ISO)).toBe(
      EXPECTED_TRIM_FORM_INDEX,
    );
  });
  ...
  it('grades the best against the course record and assembles the whole card', () => {
    ...
    expect(
      athleteRating(MALE_RUNS, { gender: Gender.male, winners, courseRecordMs: MALE_COURSE_RECORD_MS, todayIso: SCORES_TODAY_ISO }),
    ).toEqual(EXPECTED_MALE_RATING);
    expect(
      athleteRating(MALE_RUNS, { gender: null, winners, courseRecordMs: MALE_COURSE_RECORD_MS, todayIso: SCORES_TODAY_ISO }),
      'genderless — empty card',
    ).toEqual(EMPTY_RATING);
    expect(
      athleteRating([], { gender: Gender.male, winners, courseRecordMs: MALE_COURSE_RECORD_MS, todayIso: SCORES_TODAY_ISO }),
      'runless — empty card',
    ).toEqual(EMPTY_RATING);
  });
});
