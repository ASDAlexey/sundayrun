...
export const RACE_SUMMARY_ROWS: ProtocolRow[] = [
  summaryRow(1, { totalMs: 1500000, note: '' }),
  summaryRow(2, { totalMs: 1550000, note: 'Первое участие' }),
  summaryRow(3, { totalMs: 1600000, note: 'Первое участие; Дети' }),
  summaryRow(4, { totalMs: 1400000, note: 'ЛР (было 24:40); Лучший результат 2026 г.' }),
  summaryRow(5, { totalMs: 1450000, note: 'Личный рекорд' }),
  summaryRow(6, { totalMs: 1350000, note: 'Рекорд трассы' }),
  summaryRow(7, { totalMs: null, note: 'сход' }),
];
...
function summaryRow(index: number, { totalMs, note }: { totalMs: number | null; note: string }): ProtocolRow {
  return {
    index,
