import { YearBadgeActivityRow } from './badge-rarity.interface';
import { YearBadgeRarity } from './badge-rarity.type';
import { YearBadge, YearBadgeType } from './year-badges.enum';

/** Year → its first race, the new-year one; 2024 is deliberately unknown. */
export const RARITY_FIRST_EVENT_DATE_BY_YEAR: Record<string, string> = { '2025': '2025-01-05', '2026': '2026-01-04' };

export const RARITY_PARTICIPANT_COUNT = 25;

/**
 * anna: one loaded 2025 (gold + all months + the new-year race) with a dozen slow finishes —
 * «Главное — участие» too. boris: bronze 2025 and gold 2026 — owns both tiers, and his 2026
 * opens after a winter break — a comeback. vera: the new-year race in both years — still one
 * holder. grisha: a bronze 2024 whose unknown first race only rules the new-year badge out.
 */
export const RARITY_ROWS: YearBadgeActivityRow[] = [
  {
    athleteKey: 'anna',
    year: '2025',
    runCount: 50,
    monthCount: 12,
    firstRunDateIso: '2025-01-05',
    hasComeback: false,
    slowFinishCount: 12,
  },
  { athleteKey: 'boris', year: '2025', runCount: 30, monthCount: 5, firstRunDateIso: '2025-02-01', hasComeback: false, slowFinishCount: 0 },
  { athleteKey: 'boris', year: '2026', runCount: 50, monthCount: 6, firstRunDateIso: '2026-03-07', hasComeback: true, slowFinishCount: 0 },
  { athleteKey: 'vera', year: '2025', runCount: 1, monthCount: 1, firstRunDateIso: '2025-01-05', hasComeback: false, slowFinishCount: 0 },
  { athleteKey: 'vera', year: '2026', runCount: 1, monthCount: 1, firstRunDateIso: '2026-01-04', hasComeback: false, slowFinishCount: 0 },
  {
    athleteKey: 'grisha',
    year: '2024',
    runCount: 30,
    monthCount: 2,
    firstRunDateIso: '2024-06-01',
    hasComeback: false,
    slowFinishCount: 0,
  },
];

/** Out of 25 participants: 2 gold holders → 8%, and grisha's tier-worthy 2024 still counts for bronze. */
export const EXPECTED_RARITY: YearBadgeRarity = {
  [YearBadge.obsessiveGold]: 8,
  [YearBadge.obsessiveBronze]: 8,
  [YearBadge.allMonths]: 4,
  [YearBadge.newYearRace]: 8,
  [YearBadge.comeback]: 4,
  [YearBadge.cameAnyway]: 4,
};

/** The ranking holders as computed off their own sources: one course crown, two year kings. */
export const RARITY_EXTRA_HOLDERS = new Map<YearBadgeType, ReadonlySet<string>>([
  [YearBadge.courseKing, new Set(['anna'])],
  [YearBadge.yearKing, new Set(['anna', 'boris'])],
]);

/** The activity rarity plus the merged-in ranking badges: 1 of 25 → 4%, 2 of 25 → 8%. */
export const EXPECTED_RARITY_WITH_EXTRAS: YearBadgeRarity = {
  ...EXPECTED_RARITY,
  [YearBadge.courseKing]: 4,
  [YearBadge.yearKing]: 8,
};
