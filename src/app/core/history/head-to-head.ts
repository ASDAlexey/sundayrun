...
import { type HeadToHead, type HeadToHeadMeeting } from './head-to-head.interface';
...
export function buildHeadToHead(leftRuns: AthleteRun[], rightRuns: AthleteRun[]): HeadToHead {
  const rightBySlug = new Map(filterRuns(rightRuns, { year: null, distanceKm: FIVE_KM_DISTANCE_KM }).map((run) => [run.slug, run]));
  const meetings = filterRuns(leftRuns, { year: null, distanceKm: FIVE_KM_DISTANCE_KM })
