...
describe('ratingBoard', () => {
  ...
  it('ranks the gendered in-form athletes by index, rank and name, skipping the rest', () => {
    expect(ratingBoard(BOARD_RECORDS, { winners, courseRecords: BOARD_COURSE_RECORDS, todayIso: BOARD_TODAY_ISO })).toEqual(EXPECTED_BOARD);
    expect(
      ratingBoard(BOARD_RECORDS, { winners, courseRecords: EMPTY_COURSE_RECORD_HISTORY, todayIso: BOARD_TODAY_ISO }).map(
        (row) => row.localGrade,
      ),
      'recordless genders grade nobody',
    ).toEqual([null, null, null, null]);
    expect(ratingBoard([], { winners, courseRecords: BOARD_COURSE_RECORDS, todayIso: BOARD_TODAY_ISO }), 'no athletes — no board').toEqual(
      [],
    );
  });
});
