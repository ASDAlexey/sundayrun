...
/** The announcement to publish, plus who publishes it and over which transport. */
export interface SiteMetaRequest {
  readonly token: string;
  readonly meta: SiteMetaFile;
 ...
 */
export async function publishSiteMeta(request: SiteMetaRequest): Promise<string> {
  const { token, meta, fetchFn = DEFAULT_GITHUB_FETCH } = request;
  const commitSha = await commitFilesAtomically({
    token,
    fetchFn,
    buildFiles: () => Promise.resolve([{ path: SITE_META_JSON_PATH, base64Content: jsonToBase64(meta) }]),
    message: SITE_META_COMMIT_MESSAGE,
  });
