...
describe('RecordsPage', () => {
  ...
  beforeEach(() => {
    ...
    TestBed.configureTestingModule({
      providers: [
        provideRouter([]),
        { provide: AthletesService, useValue: { loadRecords, loadCourseRecords, loadFirstLapRecords, loadSeasonRuns, loadSeasonLapRuns } },
  ...
  it('renders both full leaderboards with places and the participant counters', async () => {
    ...
    expect(element.querySelectorAll('.records__board').length, 'two leaderboards, two timelines and two first-lap cards').toBe(6);
  ...
  it('search keeps the real place, and an unmatched query reports no matches', async () => {
    ...
    expect(
      ...
      'leaderboards hide behind the no-matches message, the record timelines and first-lap cards stay',
    ).toBe(4);
  });
  ...
  it('filters by season and by gender', async () => {
    ...
    expect(fixture.nativeElement.querySelectorAll('.records__board').length, 'one leaderboard, one timeline and one first-lap card').toBe(
      3,
    );
