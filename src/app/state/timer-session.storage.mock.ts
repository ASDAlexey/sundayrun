...
function payload(
  sessions: unknown,
  { activeId, schemaVersion = TIMER_SESSION_SCHEMA_VERSION }: { activeId: unknown; schemaVersion?: number },
): string {
...
export const STORED_TIMER_SESSIONS_JSON = payload([OLDER_TIMER_SESSION, TIMER_SESSION], { activeId: TIMER_SESSION_ID });
...
export const FOREIGN_SCHEMA_TIMER_SESSIONS_JSON = payload([TIMER_SESSION], {
  activeId: TIMER_SESSION_ID,
  schemaVersion: TIMER_SESSION_SCHEMA_VERSION + 1,
});

/** `sessions` hand-edited into something that is not a list. */
export const NON_ARRAY_TIMER_SESSIONS_JSON = payload('всё пропало', { activeId: null });

/** The pointer survived a measurement that did not; it must not leave the page on a ghost session. */
export const DANGLING_ACTIVE_ID_JSON = payload([TIMER_SESSION], { activeId: 'session-that-was-deleted' });

/** A pointer of the wrong type degrades exactly like a dangling one. */
export const NON_STRING_ACTIVE_ID_JSON = payload([TIMER_SESSION], { activeId: TIMER_SESSION_CREATED_AT_MS });
...
export const PARTLY_BROKEN_TIMER_SESSIONS_JSON = payload([BROKEN_STATUS_SESSION, TIMER_SESSION], { activeId: TIMER_SESSION_ID });
 ...
 */
export const LEGACY_TIMER_SESSION_JSON = payload([{ ...TIMER_SESSION, tileOrder: undefined }], { activeId: TIMER_SESSION_ID });
