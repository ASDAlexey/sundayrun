...
> **Статус: ✅ выполнено (2026-07-19).** Все фазы закрыты: `index.json`, `athletes.json` и
> `events/*/results.json` удалены из репозитория, БД — единственный источник для рантайма,
> пререндера и админ-импорта. Документ оставлен как запись решений и обоснований; актуальное
> состояние — в [ARCHITECTURE.md](./ARCHITECTURE.md) и [SQLITE_DB.md](./SQLITE_DB.md).
