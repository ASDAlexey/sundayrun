...
- **Хранение данных** — план в [SQLITE_DB.md](./SQLITE_DB.md):
  ...
  - SQLite `data/sundayrun.db` в репозитории + HTTP Range-запросы из браузера (`sqlite-wasm-http`) — точечные SQL-запросы вместо загрузки монолитных `index.json`/`athletes.json` целиком
