...
- **Prerender (SSG)**: `@angular/ssr` с `outputMode: static` — `/`, `/races`, `/records`, `/admin`
  ...
  рендеринга JS. Данные главной и `/races` фетчатся с CDN уже на этапе пререндера
  (`loadWithTransfer`: `PendingTasks` держит рендер, `TransferState` передаёт payload клиенту) —
  карточки и анонс запечены в HTML, гидратация ничего не сдвигает (CLS был 0.58 из-за появления
  контента после загрузки). Браузер применяет запечённые данные синхронно и затем обновляет их
  с CDN — публикации происходят между деплоями. Ошибка фетча на сборке молча оставляет
  loading-состояние; localStorage на сервере подменяется noop-заглушкой.
...
(каталог `data/` репозитория `sundayrun` ещё пуст — уйдёт после первой публикации; Best Practices 96 → 100).
